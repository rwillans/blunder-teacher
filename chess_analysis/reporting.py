...
def build_public_puzzle_payload(puzzles: list[PuzzleRecord]) -> list[dict[str, object]]:
    """Return the privacy-reduced payload used by the static web app."""
    payload: list[dict[str, object]] = []
    for puzzle in puzzles:
        payload.append(
            {
...
def build_public_weakness_payload(puzzles: list[PuzzleRecord]) -> list[dict[str, object]]:
    payload: list[dict[str, object]] = []
    for weakness in build_weakness_payload(puzzles):
        payload.append(
            {
                "group_type": weakness["group_type"],
                "label": weakness["label"],
                "count": weakness["count"],
                "average_eval_loss": weakness["average_eval_loss"],
                "average_eval_loss_display": weakness["average_eval_loss_display"],
                "max_eval_loss": weakness["max_eval_loss"],
                "max_eval_loss_display": weakness["max_eval_loss_display"],
                "mate_loss_count": weakness["mate_loss_count"],
                "weakness_score": weakness["weakness_score"],
                "puzzle_ids": weakness["puzzle_ids"],
            }
        )
    return payload
...
def _write_public_puzzle_payload_file(output_file: Path, puzzles: Iterable[PuzzleRecord]) -> Path:
    payload = build_public_puzzle_payload(list(puzzles))
    output_file.write_text(json.dumps(payload, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
    return output_file


def _write_public_weakness_payload_file(output_file: Path, puzzles: Iterable[PuzzleRecord]) -> Path:
    payload = build_public_weakness_payload(list(puzzles))
    output_file.write_text(json.dumps(payload, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
    return output_file
...
def write_web_public_puzzles_json(project_root: Path, puzzles: Iterable[PuzzleRecord]) -> Path | None:
    ...
    return _write_public_puzzle_payload_file(public_dir / "puzzles.json", puzzles)
...
def write_web_public_weaknesses_json(project_root: Path, puzzles: Iterable[PuzzleRecord]) -> Path | None:
    ...
    public_dir = web_dir / "public"
    public_dir.mkdir(parents=True, exist_ok=True)
    return _write_public_weakness_payload_file(public_dir / "weaknesses.json", puzzles)
